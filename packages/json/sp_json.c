...
SP_COLD static __attribute__((noinline, noreturn)) void sp_json_too_deep(jbuf *b) {
  free(b->p);  /* the walk's one buffer; nothing else is live above the raise */
...
/* Everything but a container: a single GC string, no buffer. */
...
static void sp_json_val_b(jbuf *b, sp_RbVal v, int depth) {
  ...
  if (sp_obj_to_json_fn) { const char *uj = sp_obj_to_json_fn(v); if (uj) { jb_s(b, uj); return; } }
