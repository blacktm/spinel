...
#
# Each of these builtins hoists its receiver into a C temporary, reads that
# temporary again on every turn -- its length is the loop bound and the element
# the block is handed comes out of it -- and, in map!'s case, writes back into
# it. A receiver that no Ruby name holds is exactly the one that can be
# collected while its own loop is still walking it, so every arm below calls a
# method for its receiver rather than naming it first, and every block
# allocates before it looks at what it was handed.
#
# Each arm prints the number of turns it took as well as its answer: a loop
# that ends early is a changed line rather than a silent pass.
