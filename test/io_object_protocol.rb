...
# and object_id are the identity, respond_to? asks the class's surface.
require "tmpdir"

def addr(s)
  s.sub(/0x\h{16}/, "0xADDR")
end
...
Dir.mktmpdir do |dir|
  ...
  p [sa <=> 1, f <=> "x", d <=> 1, r <=> nil, d <=> nil, sa <=> nil]
  ...
  # respond_to? asks the class, with Object's own names always true
  p [r.respond_to?(:path), r.respond_to?(:hash), r.respond_to?("to_s")]
  p [f.respond_to?(:initialize_copy), f.respond_to?(:initialize_copy, true), f.respond_to?(:flock)]
  p [d.respond_to?(:each_child), d.respond_to?(:entries), d.respond_to?(:nope), sa.respond_to?(:mtime), sa.respond_to?(:ftype)]
