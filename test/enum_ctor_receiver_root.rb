...
#
#   arm                      wrong out of
#   swept first                 12   40000    fails plain
#   inspect                      2     400    fails plain
#   array inspect                1     400    fails plain
#   StopIteration#result         0     400    plain-clean, see below
#   endless first                0     200    plain-clean, see below
#   endless first offset         0     200    plain-clean, see below
#   endless next / peek /        0     100    plain-clean, see below
#     rewind / take                    each
#
# Under GC stress at -O2 master usually does not get far enough to report at
# all: nine runs in ten abort with no output. The one run in ten that finishes
# has every arm wrong -- 3245, 3, 3, 400, 200, 200, and 100 apiece -- so the
# arms that are quiet on a plain run are carrying real coverage; their counts
# are simply not a stable thing to quote, because the abort usually comes first.
#
# All of the plain-run failures are wrong answers rather than crashes, with a
# zero exit status, which is the point: this defect does not announce itself.
