    ...
    if (pty && sp_streq(pty, "StringNode")) {
      ...
      if (t == TY_INT) {
        ...
        /* a reference-backed builtin handle (Proc, Fiber, Thread, ...):
           renders as `#<Fiber:0x... (created)>` and friends rather than
           refusing the interpolation. CRuby's #to_s for these prints the
           object address, which nothing here can reproduce; its #inspect form
           is the same shape and actually informative (#3539). */
