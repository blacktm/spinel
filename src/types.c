...
/* The IO family -- a File, IO or File::Stat handle and a Dir handle -- also
   ...
   <=> while the inference typed the slot poly, so `p f.to_s` and `f <=> f`
   refused to compile. The other heap handles keep their own to_s (Thread,
   Exception, Method) or are not audited here. */
